...
export default function StateJudgesPage() {
  ...
  const { data } = useQuery({
    queryKey: ['judges', 'SHC', stateId, divisionId],
    queryFn: () =>
      judgesAPI.list({
        court_type: 'SHC',
  ...
  // Use API data when available; fall back to static judge records
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <span>/</span>
        <Link to={`/csi/state/high-court/${stateId}`} className="hover:text-emerald-700">
          {state?.name || stateId}
        </Link>
        <span>/</span>
      ...
      {isLoading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {judges.map((judge, i) => (
            ...
            >
              <button
                onClick={() => navigate(`/csi/state/high-court/${stateId}/${divisionId}/${judge.id}`)}
                ...
                <Card className="p-4 hover:shadow-card-hover hover:border-purple-200 transition-all group border border-gray-100">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {judge.role && (
                        <p className="text-xs text-purple-600 font-medium mt-0.5">{judge.role}</p>
                      )}
                      {judge.court_number && (
      ...
      <div>
        <Link
          to={`/csi/state/high-court/${stateId}`}
