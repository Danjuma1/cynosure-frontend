...
export default function CADivisionsPage() {
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['courts', 'CA'],
    queryFn: () => courtsAPI.list({ court_type: 'CA', page_size: 50, ordering: 'name' }),
  })
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-3xl font-display font-bold text-charcoal-900">Court of Appeal</h1>
      ...
      <section>
        ...
        ) : active.length === 0 && !isError ? (
          <Card className="p-8 text-center text-gray-500">
            <BuildingLibraryIcon className="h-10 w-10 mx-auto text-gray-300 mb-2" />
            <p>No active CA divisions yet. Run the seed command to populate data.</p>
          </Card>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {active.map((court, i) => (
              ...
              >
                <Link to={`/csi/federal/CA/${court.id}`}>
