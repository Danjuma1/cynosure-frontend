...
import { NICN_DIVISIONS } from '@/data/csi/nicData'

export default function NICDivisionsPage() {
  const active = NICN_DIVISIONS.filter((d) => d.active)
  const inactive = NICN_DIVISIONS.filter((d) => !d.active)
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <span>/</span>
        <span className="text-charcoal-900 font-medium">National Industrial Court</span>
      </motion.nav>
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-white bg-orange-600 px-2 py-0.5 rounded">NICN</span>
            </div>
            <p className="text-gray-500 text-sm">
              {active.length} of {NICN_DIVISIONS.length} judicial divisions available
            </p>
          </div>
        </div>
        <p className="text-gray-600 mt-2 text-sm">
          The National Industrial Court has exclusive jurisdiction over labour, employment,
          and trade union matters. Select a division to view its judges and cause lists.
        </p>
      </motion.div>
      ...
      <section>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {active.map((division, i) => (
            ...
            >
              <Link to={`/csi/federal/NIC/${division.id}`}>
                <Card className="p-5 h-full hover:shadow-card-hover hover:border-orange-200 transition-all group border-orange-100">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <ChevronRightIcon className="h-5 w-5 text-gray-300 group-hover:text-orange-600 group-hover:translate-x-0.5 transition-all" />
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-100">
                    <span className="text-xs text-orange-600 font-medium">View Judges & Cause Lists →</span>
                  </div>
                </Card>
              </Link>
            </motion.div>
