...
export default function NICDivisionsPage() {
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['courts', 'NIC'],
    queryFn: () => courtsAPI.list({ court_type: 'NIC', page_size: 50, ordering: 'name' }),
  })
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-3xl font-display font-bold text-charcoal-900">National Industrial Court</h1>
              <span className="text-xs font-bold text-white bg-orange-600 px-2 py-0.5 rounded">NIC</span>
            </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {active.map((court, i) => (
              <motion.div key={court.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
                <Link to={`/csi/federal/NIC/${court.id}`}>
                  <Card className="p-5 h-full hover:shadow-card-hover hover:border-orange-200 transition-all group border-orange-100">
                    ...
                    <h3 className="font-semibold text-charcoal-900 group-hover:text-orange-700 transition-colors">{court.name}</h3>
                    ...
                    {court.phone_number && (
                      <p className="text-xs text-gray-400 mt-1">Tel: {court.phone_number}</p>
                    )}
                  </Card>
                </Link>
              </motion.div>
